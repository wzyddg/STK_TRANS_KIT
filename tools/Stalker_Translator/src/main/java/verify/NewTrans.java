...
public class NewTrans {
	...
	static NewTrans singelInstance = null;
	...
	public static void printHelp() {
		System.out.println("\tSTALKER Automatic Machine-Translation System 2.0 ( July 2018 )");
		...
		System.out.println(
				"7.show a formatted string containing the names of .xml files in a folder so that you can paste the string to the localization.ltx (for the mods of 'Shadow of Chernobyl').");
	...
	public static void translateScriptFile(File rus) throws Exception {
		...
		Pattern p = Pattern.compile(
				"\"([" + MyStringUtil.rusLettersString + MyStringUtil.punctuationString + " a-zA-Z0-9\\\\]*?)(?=\")");
		Matcher m = p.matcher(chsString);
		int i = 0;
		while (m.find()) {
			String thisSentence = m.group(1);
		...
		f1: for (Iterator<String> iterator = sortedList.iterator(); iterator.hasNext() || !"".equals(string);) {
			...
			try {
				...
				Pattern p1 = Pattern.compile("(?:" + MyStringUtil.actionPattern + "|" + MyStringUtil.colorPattern + "|"
						+ MyStringUtil.scriptPlaceHolderPattern + ")");
				...
				String[] pieces = string.split("(?:" + MyStringUtil.actionPattern + "|" + MyStringUtil.colorPattern
						+ "|" + MyStringUtil.scriptPlaceHolderPattern + ")");
					...
					for (int j = 0; j < pieces.length; j++) {
						...
						if (!splitter.isEmpty()) {
							if (MyStringUtil.hasLetterOrNumber(pieces[j])
									|| splitter.get(0).matches(MyStringUtil.actionPattern)
									|| splitter.get(0).matches(MyStringUtil.scriptPlaceHolderPattern)) {
	...
	public static void translateLTXFileNew(File rus) throws Exception {
		...
		Pattern p = Pattern.compile("(?<== )([" + MyStringUtil.rusLettersString + MyStringUtil.punctuationString
				+ " a-zA-Z0-9\\\\]*?)(?=\\n)");
		Matcher m = p.matcher(chsString);
		int i = 0;
		while (m.find()) {
	...
	public static String transToTarget(String sentence, String targ) throws Exception {
		...
		if (sentence.matches("[\\s0-9" + MyStringUtil.punctuationString + "]*?")) { // only numbers and punctuation
			return sentence;
		}
